function [x,functionParams,params,l,u] = gen_case_4(N,T)
...
% Case_4: infeasible start case. Tradeoff between production and decline
%first well has high CGR with high decline rate
%last well has low CGR with low decline rate
%for small T, we should fully open 1st well
%for large T, we should fully open last well
%for middle T, we should see a tradeoff
...
decline_i = linspace(2,0.05,N)';
decline = repelem(decline_i,T);

%5. CGR
CGR_i = linspace(20,15,N);
CGR = repelem(CGR_i,T)';
